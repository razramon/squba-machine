			...
			board(std::make_shared<std::pair< std::shared_ptr<Board>, std::shared_ptr<Board>>>(board)), indexOfGameShips(indexOfShips)
{
	...
	dllA = std::make_pair<std::string, std::shared_ptr<GetAlgoFuncType>>(std::move(dllNameA), std::make_shared<GetAlgoFuncType>(algoA.second));
	std::string dllNameB(algoB.first);
	dllB = std::make_pair<std::string, std::shared_ptr<GetAlgoFuncType>>(std::move(dllNameB), std::make_shared<GetAlgoFuncType>(algoB.second));
}
